...
export default function VoteButtons({ tip }: Props) {
  const { handleVote, votedTips } = useTips();
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex gap-3">
        <button onClick={() => handleVote(tip.id, "up")} className={btnClass("up")}>
          <span className="text-xl">👍</span>
          Fungovalo mi
        </button>
        <button onClick={() => handleVote(tip.id, "down")} className={btnClass("down")}>
          <span className="text-xl">👎</span>
          Nefungovalo mi
        </button>
      </div>
